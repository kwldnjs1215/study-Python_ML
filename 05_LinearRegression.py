...
다항회귀모형(Polynomial regression model)  
 - X와 y변수가 비선형관계를 가지는 경우 고차항을 적용한 회귀모형 
 1차항 : 선형관계 
 2차항 : U자관계
 3차항 : S자관계   
"""
...
## 단계1 : 데이터 준비 : 칼럼명 없음 
df = pd.read_csv(r'C:\ITWILL\4_Python_ML\data\auto-mpg.csv',header=None)
...
# 종속변수 vs 독립변수 선형성 확인
sns.pairplot(data = new_df) # 산점도 행렬 
'''
mpg vs cylinders : 선형관계 
mpg vs horsepower, weight : 비선형관계 
''' 
...
# 선형다중회귀모델 score : 0.6939048496695597
...
X1_poly.shape # (392, 3)
# [1.0000e+00, 1.3000e+02, 1.6900e+04] -> 절편, x1(1차), x2(2차)

X2_poly = poly.fit_transform(X[['weight']]) # weight 변수 2차항
X2_poly.shape # (392, 3)
# [1.0000000e+00, 3.5040000e+03, 1.2278016e+07] -> 절편, x1(1차), x2(2차)
'''
a : 기울기
b : 절편 
y = cylinders*a + horsepower*a + horsepower^2*a + b + weight*a + weight^2*a  + b
'''


# 단계2 : 다항식 변수 추가 
X_new = X.copy() # 내용 복사  
X_new.columns # ['cylinders', 'horsepower', 'weight']

X_new['horsepower_poly'] = X1_poly[:, 2] # horsepower 변수 2차항 추가 
X_new['weight_poly'] = X2_poly[:, 2] # weight 변수 2차항 추가
X_new.columns # ['cylinders', 'horsepower', 'weight', 'horsepower_poly', 'weight_poly']

X_new.columns = ['cylinders', 'horsepower', 'horsepower_poly', 'weight', 'weight_poly']
...
# 다항회귀모델 score : 0.7555885273961866
